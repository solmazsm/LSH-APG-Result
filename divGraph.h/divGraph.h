...
//solmaz
std::ofstream logFile("lsh_apg_metrics_deep1M_v1.csv", std::ios::out | std::ios::trunc);
...
//	int localDensity = linkLists[pId]->size(); // Number of neighbors for this node
	...
	if (checkedArrs_local.find(pId) == checkedArrs_local.end()) {
		if (logFile.is_open()) {
	logFile << "Candidate ID=" << pId 
            << ", InsertionTime=" << insertionTime << "ns"
	//		if (linkLists[pId]) {
	//			logFile << ", LocalDensity=" << linkLists[pId]->size();
	//		}
             << std::endl;
    logFile.flush();} else {
		std::cerr << "Error: logFile is not open!" << std::endl;
	}
	//end solmaz
}
}
...
void divGraph::showInfo(Preprocess* prep)
{
	...
	if (fp) fprintf(fp, "%s\nT=%d,L=%d,K=%d\ndist=%f, cnt=%f, unique=%d, std=%f, Recall=%f\ncc=%f, pruning=%f, IndexingTime=%f s.\n\n", file.c_str(), T, L, K, dist_total / cnt1, (float)cnt / N1, (int)f, sigma, (float)rec / cnt1,
		...
		//solmaz start
		std::cout << "Total nodes in the graph: " << N << std::endl;
		logFile << "Total nodes in the graph: " << N << std::endl;
		std::cout << "Total nodes in linkLists: " << linkLists.size() << std::endl;
		logFile << "Total nodes in linkLists: " << linkLists.size() << std::endl;
		int activeNodes = 0;
for (const auto& node : linkLists) {
    if (node->size() > 0) {
        activeNodes++;
    }
}
std::cout << "Active nodes with neighbors: " << activeNodes << std::endl;
logFile << "Active nodes with neighbors: " << activeNodes << std::endl;
size_t totalEdges = 0;
for (const auto& node : linkLists) {
    totalEdges += node->size();
}
std::cout << "Total edges in the graph: " << totalEdges << std::endl;
logFile << "Total edges in the graph: " << totalEdges << std::endl;
logFile.flush();
